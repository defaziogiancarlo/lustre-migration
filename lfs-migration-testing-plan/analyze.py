...
def get_pid_paths(directory):
    '''get the paths of all the files that are integers'''
    dir_path = pathlib.Path(directory)
    all_paths = dir_path.glob('*')
    return [p for p in all_paths if is_int(p.name)]


def consolidate(directory):
    '''consolidate the individual process logs into a single
    file and compress them.
    '''
    # look for file that says results
    dir_path = pathlib.Path(directory)
    results_path = dir_path / 'results'
    # TODO what if it exists and is not a file?
    if results_path.is_file():
        return

    all_data = {}

    for pid_path in get_pid_paths(directory):
        with open(pid_path, 'r') as f:
            all_data[pid_path.name] = yaml.safe_load(f)
...
def make_parser():
    ...
    parser.add_argument(
        ...
        '--logs-directory',
        required=True,
        help='the directory of the logs to summarize',
    )
    parser.add_argument(
        '-o',
        '--output-file',
        help='the file to write the results to, the default is stdout',
    )
    parser.add_argument(
        '-c',
        '--consolidate',
