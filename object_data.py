...
'''The object data tests

The basic idea is to create a bunch of files, then stripe them (mov to some ost or osts)
...
size per file, total size
initial OST configuration, final OST configuration
...
def delete_dir(dirname):
    '''Use drm to delete a dir, you need
    mpifileutils for this.
    '''
    # N=4 and n=64 don't seem to segfault
    # for other N,n combos, been getting segfaults
    subprocess.run(
        ['drm' '-N4', '-n64', dirname],
        check=True,
    )
